...
class InterviewHistoryRepository extends AbstractRepository implements InterviewHistoryRepositoryInterface
{
    ...
    public function getList(?int $company_id): array
    {
        ...
        return InterviewHistory::query()
            ->select([
                ...
                'students.name_kana as student_name_kana',
            ...
            ->leftJoin('students', 'students.id', 'interview_histories.student_id')
            ->leftJoin('users', 'users.id', 'interview_histories.user_id')
            ->leftJoin('teacher_schedules', 'teacher_schedules.user_id', 'users.id')
            ->leftJoin('schedules', 'schedules.id', 'teacher_schedules.schedule_id')
            // ->where('company_id', $company_id)
            ->groupBy(
                'interview_histories.student_id',
                'students.name',
    ...
    //         ->leftJoin('teacher_schedules', 'teacher_schedules.user_id', 'users.id')
    //         ->leftJoin('schedules', 'schedules.id', 'teacher_schedules.schedule_id')
    //         ->where('interview_histories.company_id', $company_id)
    //         ->groupBy(
    //             'interview_histories.student_id',
    //             'students.name',
    //             'students.name_kana',
