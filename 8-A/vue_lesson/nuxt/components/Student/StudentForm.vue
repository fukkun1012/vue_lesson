    ...
    >
        <v-container>
            <v-row dense>
                    ...
                    <v-card outlined class="mb-3" v-if="student_flg">
                        <v-card-text class="px-2 py-1">
                            <v-row dense>
                                <v-col cols="6">
                                    <v-row dense>
                                    <v-col cols="12" class="overline red--text">修正者</v-col>
                                    <v-col cols="12" class="red--text">{{ data.name }}</v-col>
                                    </v-row>
                                </v-col>

                                 <v-col cols="6">
                                    <v-row dense>
                                        <v-col cols="12" class="overline">コース</v-col>
                                        <v-col cols="12">{{ data.course_name }}</v-col>
                                    </v-row>
                                </v-col>

                                <v-col cols="6">
                                    <v-row dense>
                                        <v-col cols="12" class="overline">開始日</v-col>
                                        <v-col cols="12">{{ $moment(data.start_date).format('YYYY年M月D日') }}</v-col>
                                    </v-row>
                                </v-col>
                                <v-col cols="6"
                                       v-if="data.end_date"
                                >
                                    <v-row dense>
                                        <v-col cols="12" class="overline">終了日</v-col>
                                        <v-col cols="12">{{ $moment(data.end_date).format('YYYY年M月D日') }}</v-col>
                                    </v-row>
                                </v-col>
                                <v-col cols="6">
                                    <v-row dense>
                                        <v-col cols="12" class="overline">ステータス</v-col>
                                        <v-col cols="12">
                                            {{ $selects.after_graduation_flg[data.after_graduation_flg].text }}
                                        </v-col>
                                    </v-row>
                                </v-col>
                            </v-row>
                ...
                >
                    <date
                        ...
                        :clearable="true"
                        :rules="[v => $validation.required(v,'ステータスが受講中ではない場合終了日')]"
                        hint="必須"
                        :persistent-hint="true"
                    ></date>
                </v-col>
            ...
            <v-row class="justify-center">
                <v-btn dark :color="$colors.main" @click="sendForm(false)">{{ data.id === undefined ? '登録' : '更新' }}</v-btn>
                <delete-button
...
export default {
    name: "StudentForm",
    components: {DeleteButton, PasswordChange, SelectInput, Date, AutocompleteWithApi, TextInput},
    props: {
        value: {
            type: Object,
            required: true
    ...
    data() {
        return {
            ...
            student_flg: false,
            admin_flg: false,
            course_group_url: this.$utils.getApiUrl(this.$apis.autocomplete.course_group_student, true)
        }
    },
    watch: {
        value(v) {
    ...
    created() {
        ...
        if (this.$utils.getThemeClass() === 'admin') {
            this.admin_flg = true;
            this.course_group_url = this.$utils.getApiUrl(this.$apis.autocomplete.course_group, true, true);
        }
        if (this.$utils.getThemeClass() === 'master') {
            if (this.$route.path.match('/company')) {
                this.course_group_url = this.$utils.getApiUrl(this.$apis.autocomplete.course_group, true,);
            }
        }
        if(!this.data.after_graduation_flg){
            this.data.after_graduation_flg = 0;
