                ...
                >
                    <div class="headers">
                        ...
                        <div class="student name">
                            氏名{{ $utils.isAdmin() && !company_search ? '(企業名)':'' }}
                        </div>
                        ...
                        <div class="date reservations"
                             ...
                             :class="{ 'today-header': $moment(start).add(n-1,'day').isSame($moment(), 'day') }"
                        >
                        </div>
                    </div>
                    <v-skeleton-loader
                    ...
                    <div
                        v-else
                        class="items" v-for="(student,key) in students"
                        :key="key"
                    >
                        <div class="student attendance_flg">
                        ...
                        <div class="student start_date">
                            {{ $moment(student.start_date).format('YYYY年M月D日') }}
                        </div>
                        <div class="student name">
                            <v-btn
                                text
                                color="light-blue"
                                @click="$utils.openNewTab($utils.getHomeUrl()+'/student/' + student.id)"
                            >
                                {{ student.name }}
                                {{$utils.isAdmin() && !company_search ? '('+student.company_name+')':''}}
                            </v-btn>
                        </div>
                        ...
                        <reservations
                            ...
                            :key="n"
                            :student="student"
                            :date="$moment(start).add(n-1,'day').format('YYYY-MM-DD')"
                            :class="{ 'today-border': $moment(start).add(n-1,'day').isSame($moment(), 'day') }"
                        ></reservations>
                    </div>
                </div>
                <div
                    ...
                    :style="loaderWidth"
                >
                    <infinite-loading ref="infinite_loader" @infinite="infiniteHandler" spinner="spiral">
                        <div slot="spinner">
                            <v-skeleton-loader
                                height="30"
                                tile
...
export default {
    ...
    components: {AutocompleteWithApi, Reservations, Month, TextInput, SelectInput, Date},
    created(){
        this.offset = 0;
        this.setFromTo()
        this.getStudentProgresses(true,true);
        if (this.$utils.isUser()) {
            this.curriculum_url = this.$utils.getApiUrl(this.$apis.autocomplete.curriculum_group, true);
        } else if (this.$utils.isAdmin()) {
            this.curriculum_url = this.$utils.getApiUrl(this.$apis.autocomplete.curriculum, true);
        }
    },
    mounted() {
        this.setWidth();
    ...
    methods: {
        ...
        getDateHtml(n) {
            ...
            const isToday = day.isSame(this.$moment(), 'day');
            let result = '<p class="week_' + day.format('d') + (isToday ? ' today' : '') +'">';
            if (n === 1 || day.date() === 1) {
                result += day.format('YYYY年M月') + '<br>';
            }
            result += '' + day.format('D日') + '(' + this.$utils.getWeekStr(day) + ')</span></p>';
            return result;
        },
        setWidth() {
        ...
        async getStudentProgresses(search=false,reset=false) {
            try {
                ...
                const result = await this.$axios(this.$utils.getApiUrl(this.$apis.progress_students, true,true),{params:params})
                console.log(result);
                if(this.offset === 0){
                    // 初期表示または検索実行時
