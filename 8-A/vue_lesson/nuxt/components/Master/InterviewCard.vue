<template>
    <v-card outlined class="fill-height" :loading="loading" :disabled="loading">
        <v-card-title
            >面談履歴<v-spacer /><v-btn
                v-if="!$utils.isStudent()"
                outlined
                @click="clickRow({ student_id: $route.params.id })"
                >新規作成</v-btn
            ></v-card-title
        >
        ...
        <v-expand-transition>
            <div v-if="!loading">
                ...
                <v-simple-table v-else fixed-header :height="196">
                    <thead>
                        <tr>
                            <th>面談日</th>
                            <th>営業評価</th>
                            ...
                            <template v-slot:activator="{ on, attrs }">
                                ...
                                >
                                    ...
                                    <td>
                                        <v-icon
                                            x-small
                                            v-for="i in item.sales_evaluation"
                                            :key="i"
                                    ...
                                    <td>
                                        {{ getReason(item.evaluation_reason) }}
                                    </td>
                                </tr>
                            </template>
                            <span
                                v-html="
                                    item.evaluation_reason.replace(
                                        /\r?\n/g,
                                        '<br>'
                                    )
                                "
...
export default {
    ...
    methods: {
        async getData() {
            try {
                this.loading = true;
                let url = this.$utils.getApiUrl(
                    this.$apis.interview_history_student,
                    true,
                    true
                );
